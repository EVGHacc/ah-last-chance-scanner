...
def main():
    ...
    valid=status in ('OK','OK_ZERO_ROWS') and auth_mode=='user-refresh' and canonical_delay<=240
    obs={'schemaVersion':4,'date':date,'weekday':canonical.strftime('%A'),'scheduledSlot':slot,'scheduledAt':canonical.isoformat(),'rawScheduledSlot':raw_scheduled.strftime('%H:%M'),'rawScheduledAt':raw_scheduled.isoformat(),'startedAt':started.isoformat(),'completedAt':completed.isoformat(),'checkedAt':completed.isoformat(),'delaySeconds':canonical_delay,'rawDelaySeconds':raw_delay,'status':status,'valid':valid,'official1925':slot=='19:25','authMode':auth_mode,'categories':['Vlees','Bakkerij'],'stores':stores}
